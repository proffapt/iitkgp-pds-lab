...
int main()
{
	...
	// Displaying data having same age:
	printf("Displaying data with same age:\n");
	int k = 0;
	int count = 0;
	int i_count[20];
	for (int i = 0; i < 20; i++)
	...
	int same_age_index1[20], same_age_index2[20];
	for (int i = 0; i < 20; i++)
	{
		for (int j = 0; j < 20; j++)
		{
			if (age[j] == age[i] && j != i)
			{
				...
				{
					// storing same age index numbers in arrays
					same_age_index1[k] = i;
					same_age_index2[k] = j;
					k++;
	...
	if (count != 0)
	{
		...
		for (int i = 0; i < k; i++)
		{
			printf("\nRoll: %d Age: %d Mark: %d", roll[same_age_index1[i]], age[same_age_index1[i]], marks[same_age_index1[i]]);
			printf("\nRoll: %d Age: %d Mark: %d\n", roll[same_age_index2[i]], age[same_age_index2[i]], marks[same_age_index2[i]]);
			printf("*********************");
		}
		// just to remove the ***** for the last entry
	...
	else
	{
		...
		printf("\nUnfortunately there's no data with same age.\n");
	}
