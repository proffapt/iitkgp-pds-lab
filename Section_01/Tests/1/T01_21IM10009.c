...
int main()
{
	...
	do {
		...
		if (n<=0) printf("Please make sure that the range belongs to positive integers only!\n");
	}while(n<=0);
	...
	printf("Required Constructed integers is/are: ");
			...
			if(!i_flagged[a] && nums_sorted[a] != 0){
				for(int b = a+1; b < index; b++){
					if (!i_flagged[b] && nums_sorted[b] != 0){
						int A = nums_sorted[a], B = nums_sorted[b];
						// making sure A < B
						if (A > B){
							A = A + B;
							B = A - B;
	...
	for(int i = 0, is_prime = 1; i < total; i++, is_prime = 1){
		if(!i_flagged[i]){
			if(nums_sorted[i] <= 1) { is_prime = 0; continue; }
			for(int w = 2; w <= nums_sorted[i]/2; w++){
				if (nums_sorted[i]%w == 0){ is_prime = 0; break; }
			}
			if(is_prime) {
